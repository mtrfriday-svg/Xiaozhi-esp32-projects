...
void Application::Initialize() {
    ...
    // Load the speech models from the assets partition BEFORE the audio engine
    // can be initialized. On ESP32-S3/AfeAudioEngine, a missing model list
    // makes the engine fall back to the legacy "model" partition and produces
    // "MODEL_LOADER: Can not find model in partition table".
    //
    // This is especially important on the 4MB Super Mini layout, where the
    // WakeNet model is intentionally stored inside the "assets" partition.
    auto& assets = Assets::GetInstance();
    if (assets.partition_valid()) {
        if (!assets.Apply(false)) {
            ESP_LOGW(TAG, "Failed to load assets/speech models before audio init");
        } else {
            ESP_LOGI(TAG, "Assets/speech models loaded before audio initialization");
        }
    } else {
        ESP_LOGW(TAG, "Assets partition is not available before audio initialization");
    }
