...
bool Assets::LoadSrmodelsFromIndex(Assets* assets, cJSON* root) {
    // The audio engine keeps a pointer to the speech-model list. Once the
    // engine has initialized, replacing that list would leave the engine
    // holding a freed pointer. The Super Mini now loads the model before
    // audio initialization, so later Apply() calls must keep that list.
    if (assets->models_list_ != nullptr) {
        ESP_LOGI(TAG, "Speech models already loaded; keeping existing model list");
        return true;
    }
